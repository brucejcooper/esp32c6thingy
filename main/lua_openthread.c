...
static int udp_reply(lua_State *L) {
    // Arg 1 is the source message.
    // Arg 2 is the string body we wish to respond with. 
    size_t sz;
...
void udpCallback(void *aContext, otMessage *aMessage, const otMessageInfo *aMessageInfo) {
    ...
    otMessageInfo *cpy = (otMessageInfo *) lua_newuserdata(L, sizeof(otMessageInfo));
    memcpy(cpy, aMessageInfo, sizeof(otMessageInfo));
    lua_settable(L, -3);

    // Method to allow the handler to respond to the caller.
    lua_pushstring(L, "reply");
    lua_pushcfunction(L, udp_reply);
    lua_settable(L, -3);
